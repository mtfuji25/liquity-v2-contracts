...
contract CurveProxy is PrismaOwnable {
    ...
    modifier ownerOrVoteManager() {
        require(
            msg.sender == voteManager || msg.sender == owner(),
            "Only owner or vote manager"
        );
        _;
    }
    ...
    modifier onlyApprovedGauge(address gauge) {
        require(
            perGaugeApproval[msg.sender] == gauge ||
                msg.sender == depositManager,
            "Not approved for gauge"
        );
        _;
    }
    ...
    function setExecutePermissions(
        ...
        mapping(bytes4 => bool) storage _executePermission = executePermissions[
            caller
        ][target];
    ...
    function lockCRV() external returns (bool) {
        uint256 maxUnlock = ((block.timestamp / WEEK) * WEEK) +
            MAX_LOCK_DURATION;
    ...
    function mintCRV(
        address gauge,
        address receiver
    ) external onlyApprovedGauge(gauge) returns (uint256) {
    ...
    ) external ownerOrVoteManager returns (bool) {
    ...
    function execute(
        ...
        if (msg.sender != owner()) {
            ...
            require(
                executePermissions[msg.sender][target][selector],
                "Not permitted"
            );
        }
